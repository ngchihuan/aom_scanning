...
READPROG='/home/qitlab/programs/timestamp3/readevents3_withblanks'
...
def folder(directo,base_folder='/home/qitlab/data/timestamp'):
    currentdate = str(datetime.date.today()).split('-')
    year = currentdate[0]
    month = currentdate[1]
    day =  currentdate[2]
    directory = base_folder +'/'+directo+ '/'+ year + '/'  + month + '/' + day  + '/'
    if not os.path.exists(directory):
        os.makedirs(directory)
    return directory
    

def filename(optional_name,directo):
    currenttime = time.strftime( "%H:%M:%S")
    directory = folder(directo)
    if optional_name == '':
...
def stop():
    pid=(sp.check_output(["pgrep -f readevents3_withblanks"],shell=True).decode('utf-8')).split()
    for i in range(len(pid)):
        sp.call(['kill -9 '+str(pid[i])],shell=True)
    time.sleep(1)
    print(pid)
